    ...
    const handleExportPdf = () => {
        ...
        try {
            ...
            const tableData = piezas.map(p => [
                p.codigo,
                p.descripcion || '-',
                getUsuarioNombre(p.usuarioId),
                getEstatusNombre(p.estatusId),
                p.fechaRegistro.toLocaleDateString()
            ]);
            ...
            const fileName = `informe_piezas_${new Date().toISOString().slice(0, 10)}.pdf`;
            doc.save(fileName);
        } catch (err) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all"
                        >
                    ...
                    <div>
                        ...
                        <input
                            type="date"
                            value={filters.fechaDesde ? new Date(filters.fechaDesde).toISOString().slice(0, 10) : ''}
                            onChange={(e) => setFilters({ ...filters, fechaDesde: e.target.value ? new Date(e.target.value) : null })}
                            className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all"
                        />
                    </div>
                    <div>
                        ...
                        <input
                            type="date"
                            value={filters.fechaHasta ? new Date(filters.fechaHasta).toISOString().slice(0, 10) : ''}
                            onChange={(e) => setFilters({ ...filters, fechaHasta: e.target.value ? new Date(e.target.value) : null })}
                            className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all"
                        />
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-4 mt-4">
                    <div className="flex items-center gap-2">
                        ...
                        <select
                            ...
                            onChange={(e) => setFilters({ ...filters, orderBy: e.target.value as ReporteFilters['orderBy'] })}
                            className="px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all text-sm"
                        >
                        ...
                        <select
                            ...
                            onChange={(e) => setFilters({ ...filters, orderDesc: e.target.value === 'desc' })}
                            className="px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all text-sm"
                        >
                    ...
                    <div className="flex gap-2 ml-auto">
                        <button
                            ...
                            className="inline-flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-orange-500 to-amber-600 text-white font-medium rounded-xl shadow-lg shadow-orange-500/25 hover:shadow-xl hover:shadow-orange-500/30 transition-all"
                        >
                        ...
                        <button
                            ...
                            disabled={exporting || piezas.length === 0}
                            className="inline-flex items-center gap-2 px-4 py-2.5 bg-violet-600 text-white font-medium rounded-xl shadow-lg shadow-violet-500/25 hover:shadow-xl hover:shadow-violet-500/30 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                        >
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 shadow-lg overflow-hidden">
                <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700">
                    <h3 className="font-semibold text-slate-800 dark:text-white">
                        Vista Previa ({piezas.length} registros)
                    </h3>
                </div>
                {loading ? (
