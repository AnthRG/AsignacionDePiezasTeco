        ...
        <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 space-y-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            <button onClick={handleSearch} className="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 font-medium rounded-xl hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-all">
                                <Search className="w-4 h-4" />
                                Buscar
                            </button>
                            <button onClick={handleClearForm} className="inline-flex items-center gap-2 px-4 py-2.5 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 font-medium rounded-xl hover:bg-slate-200 dark:hover:bg-slate-600 transition-all">
                    ...
                    <div className="flex flex-col gap-4">
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Link de imagen (Google Drive)</label>
                            ...
                            <p className="text-xs text-slate-500 mt-1">Pega el enlace de compartir de Google Drive</p>
                        </div>
                        <div className="w-full border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-800/50 overflow-hidden">
                            {photoPreview ? (
                                <div className="relative w-full h-full group">
                                    <img src={photoPreview} alt="Vista previa" className="w-full object-contain" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; setPhotoPreview(null); }} />
                                    <button onClick={() => { setPhotoPreview(null); setFormData({ ...formData, fotoUrl: null }); }} className="absolute top-2 right-2 p-1 bg-red-500 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity" title="Quitar imagen">
                                        <X className="w-3 h-3" />
                                    </button>
                                </div>
                            ) : (
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg shadow-slate-200/50 dark:shadow-slate-900/50 p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <input type="text" value={filters.codigo || ''} onChange={(e) => setFilters({ ...filters, codigo: e.target.value })} onKeyDown={handleFilterKeyDown} placeholder="Código" className="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-violet-500 focus:border-transparent transition-all" />
                    <select value={filters.usuarioId || ''} onChange={(e) => setFilters({ ...filters, usuarioId: e.target.value || null })} className="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white focus:ring-2 focus:ring-violet-500 focus:border-transparent transition-all">
                        <option value="">Todos los usuarios</option>
                        {usuarios.map((u) => (<option key={u.id} value={u.id}>{u.nombre}</option>))}
                    </select>
                    <select value={filters.estatusId || ''} onChange={(e) => setFilters({ ...filters, estatusId: e.target.value || null })} className="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white focus:ring-2 focus:ring-violet-500 focus:border-transparent transition-all">
                        <option value="">Todos los estatus</option>
                        {estatus.map((e) => (<option key={e.id} value={e.id}>{e.nombre}</option>))}
                    </select>
                    <div className="flex gap-2">
                        <button onClick={handleApplyFilters} className="flex-1 px-4 py-2.5 bg-gradient-to-r from-violet-500 to-purple-600 text-white font-medium rounded-xl transition-all">Aplicar</button>
                        <button onClick={handleClearFilters} className="px-4 py-2.5 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 font-medium rounded-xl hover:bg-slate-200 dark:hover:bg-slate-600 transition-all">Limpiar</button>
                    </div>
                </div>
            </div>
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg shadow-slate-200/50 dark:shadow-slate-900/50 overflow-hidden">
                {loading ? (
                    ...
                    <div className="text-center py-12 text-slate-500 dark:text-slate-400">No hay piezas registradas</div>
                ) : (
                        ...
                        <table className="w-full">
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                {piezas.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((pieza) => (
                                    <tr key={pieza.codigo} onClick={() => loadPieceIntoForm(pieza)} className="cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-slate-500 dark:text-slate-500 text-sm">{pieza.fechaRegistro.toLocaleDateString()}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
